...
func soln1() {
	fmt.Printf("Soln1: Total trees = %d\n", treeCount(3, 1))
}

func soln2() {
	c := treeCount(1, 1) * treeCount(3, 1) * treeCount(5, 1) * treeCount(7, 1) * treeCount(1, 2)
	fmt.Printf("Soln2: Total tree = %d\n", c)
}
...
func treeCount(xInc, yInc int) int {
	i := input()
	w := len(i[0])
	c := 0
	for x, y := 0, 0; y < len(i); {
...
//
//     Right 1, down 1.  Right 3, down 1. (This is the slope you already checked.) Right 5, down 1.  Right 7, down 1.
//     Right 1, down 2.
//
